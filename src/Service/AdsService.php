...
class AdsService
{
    ...
    const TYPE = [
        ...
        'host' => [
            'name' => 'Сервер',
            'title' => 'Хочешь друзей?',
            'url' => '//regiment.zbara.dev/ads?ref='
        ]
    ];
    ...
    public function user(): string
    {
        ...
        if(isset($ads)){
            ...
            return  '<div style="border: solid 1px black; padding: 1px;">' . self::TYPE[$ads->getType()]['title'] . ': <a target="_blank" href="https://regiment.zbara.dev/ads/views/' . $ads->getId() . '">' . $ads->getName() . '</a>  —  <span style="color: #' . $color[array_rand($color, 1)] .'">' . $ads->getMessages() . '</span></div>';
        }
        return '';
    }
}
